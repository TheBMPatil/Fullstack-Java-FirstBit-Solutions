...
int main() {
	...
	for (int i = 0; i < 4; i++) {
		...
		//SalesManager* sm=(SalesManager*)emp[i];

		cout<<"\nThe type of currret index value is "<<typeid(*emp[i]).name()<<"\n";
		cout<<"\nThe type of currret index value is "<<typeid(SalesManager).name()<<"\n";


		if(strcmp(typeid(*emp[i]).name(),typeid(SalesManager).name())==0) {
		...
		if(strcmp(typeid(*emp[i]).name(),typeid(HR).name())==0) {
			{
